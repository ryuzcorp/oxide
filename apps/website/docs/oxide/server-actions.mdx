...
On [`preset: "worker"`](/oxide/configuration#preset), these are the Worker `env` and `ctx` from `fetch(request, env, ctx)`. On the Node fetch preset they are `undefined` unless you pass [`env`](/oxide/configuration#env):
