...
Clients see application error `-32000` with the tag/message. Returning `new NotFound(...)` as a **value** stays local (`T | Error`); it does not auto-promote over RPC — `Effect.fail` or throw the tagged error.

### Effect handlers and services

`action()` accepts Promise, `async function*`, `Effect`, and (with `{ stream: true }`) Effect `Stream`. Promise handlers stay the default. Effect handlers can yield `OxideRequest` / `OxideCtx` — the same data as `useRequest()` / `useCtx()`:
...
Handlers run under an `oxidejs.action` span annotated with `rpc.method`. Provide an Effect tracer/logger at the host if you want spans collected.
...
On [`preset: "celld"`](/oxide/configuration#preset), these are the Worker `env` and `ctx` from `fetch(request, env, ctx)`. On the Node fetch preset they are `undefined` unless you pass [`env`](/oxide/configuration#env):
...
Use `transport: "ws"` for live queries and the mutation queue. Node/`fetch` needs `crossws`; [`preset: "celld"`](/oxide/configuration#preset) uses `WebSocketPair`:
