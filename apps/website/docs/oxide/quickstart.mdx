...
Use `"worker"` for Cloudflare Workers (a self-hosted Workers runtime).
...
[`preset: "worker"`](/oxide/configuration#preset) writes `dist/wrangler.jsonc` and skips asset serving (`ASSETS` does that). See [`wrangler`](/oxide/configuration#wrangler).
