...
export default defineConfig({
  plugins: [oxide(), cloudflare(withOxide())],
});
```

Production builds that emit `dist/ssr/wrangler.json` also write a celld-ready `dist/wrangler.json`. See [`preset`](/oxide/configuration#preset) and [Worker](/oxide/worker).
...
- [Worker](/oxide/worker) — workflows, queues, schedules
- [Configuration](/oxide/configuration) — `preset`, paths, actions, celld
