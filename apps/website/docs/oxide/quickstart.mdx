...
Add a root `wrangler.jsonc` (preset defaults to `"worker"`) and `@cloudflare/vite-plugin`.
...
See [`preset`](/oxide/configuration#preset).
...
- [Configuration](/oxide/configuration) — `preset`, paths, actions
